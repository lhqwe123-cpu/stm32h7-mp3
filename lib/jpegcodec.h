...
/**
 * @brief       JPEG 硬件解码为 RGB565
 * @param       jpeg_data   : JPEG 源数据指针
 * @param       jpeg_size   : JPEG 数据大小(字节)
 * @param       dst_rgb565  : 输出 RGB565 缓冲区
 * @param       dst_width   : 输出缓冲区最大宽度
 * @param       dst_height  : 输出缓冲区最大高度
 * @param       out_width   : 输出实际图片宽度 (可为 NULL)
 * @param       out_height  : 输出实际图片高度 (可为 NULL)
 * @retval      0:成功, 其他:失败
 */
