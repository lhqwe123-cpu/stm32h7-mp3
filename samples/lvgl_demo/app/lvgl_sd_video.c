...
{
    sd_video_ctx_t *ctx = (sd_video_ctx_t *)timer->user_data;
    uint16_t w = 0U;
    uint16_t h = 0U;
    ...
    ctx->img_dsc.header.always_zero = 0;
    ctx->img_dsc.header.reserved = 0;
    ...
    lv_img_set_src(ctx->img_obj, &ctx->img_dsc);
...
static void create_play_controls(sd_video_ctx_t *ctx, lv_obj_t *parent)
{
    ...
    /* 返回按钮 */
    ctx->back_btn = lv_btn_create(ctx->play_ctrl_bar);
    ...
    /* / */
    ctx->pause_btn = lv_btn_create(ctx->play_ctrl_bar);
    ...
    lv_obj_set_style_text_font(ctx->replay_label, &lv_font_montserrat_12, 0);
...
static int video_play(const char *path)
{
    ...
    g_video.img_obj = lv_img_create(scr);
...
static void lvgl_sd_video_show_list(void)
{
    ...
    {
        lv_obj_t *bg = lv_img_create(g_list_scr);
    ...
    /* 返回按钮 */
    lv_obj_t *back_btn = lv_btn_create(g_list_scr);
