...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    GT1151Q_Status_e state;

    state = GT1151Q_ReadData(&Touch_dev);

    if (state == GT1151Q_OK)
    {
        if (Touch_dev.state == GT1151Q_TOUCH_DOWN || Touch_dev.state == GT1151Q_TOUCH_LONG_PRESS)
        {
            last_x = Touch_dev.points[0].x;
            last_y = Touch_dev.points[0].y;
            data->point.x = last_x;
            data->point.y = last_y;
