...
/* FATFS 实际挂载点，与 Zephyr 侧 f_mount 的 mnt_point 保持一致 */
#define FS_FATFS_MOUNT_POINT "SD:"

/**
 * @brief  将 LVGL 路径 "S:/xxx" 转换为 FATFS 路径 "SD:/xxx"
 * @param  lv_path：LVGL 传入的路径 (如 "S:/PICTURE/logo.bin")
 * @param  fatfs_path：输出缓冲区，存放转换后的 FATFS 路径
 * @param  fatfs_path_len：输出缓冲区大小
 * @retval 转换后的路径指针 (即 fatfs_path)
 */
static const char *lv_path_to_fatfs(const char *lv_path, char *fatfs_path, size_t fatfs_path_len)
{
    /* LVGL 路径格式: "S:/xxx" -> FATFS 路径格式: "SD:/xxx" */
    if (lv_path[0] == LV_FS_FATFS_LETTER && lv_path[1] == ':')
    {
        snprintf(fatfs_path, fatfs_path_len, "%s%s", FS_FATFS_MOUNT_POINT, &lv_path[2]);
    }
    else
    {
        strncpy(fatfs_path, lv_path, fatfs_path_len - 1);
        fatfs_path[fatfs_path_len - 1] = '\0';
    }
    return fatfs_path;
}
...
static void *fs_open(lv_fs_drv_t *drv, const char *path, lv_fs_mode_t mode)
{
    ...
    /* 将 LVGL 路径 "S:/xxx" 转换为 FATFS 路径 "SD:/xxx" */
    char fatfs_path[256];
    lv_path_to_fatfs(path, fatfs_path, sizeof(fatfs_path));

    FRESULT res = f_open(f, fatfs_path, flags);
    if (res == FR_OK)
    {
        return f;
    }
...
static void *fs_dir_open(lv_fs_drv_t *drv, const char *path)
{
    ...
    FRESULT res = f_opendir(d, fatfs_path);
    if (res != FR_OK)
    {
        lv_mem_free(d);
        d = NULL;
