 ...
 * @return UTF-8 coded character in Little Endian to be compatible with C chars (e.g. '', '')
 */
static uint32_t lv_txt_unicode_to_utf8(uint32_t letter_uni)
{
...
/**
 * Convert a wide character, e.g. '' little endian to be UTF-8 compatible
 ...
 * E.g. in "ART" index of 'R' is 2th char but start at byte 3 because '' is 2 bytes long
 ...
 * @param letter_uni an Unicode letter
 * @return ISO8859-1 coded character in Little Endian to be compatible with C chars (e.g. '', '')
 */
static uint32_t lv_txt_unicode_to_iso8859_1(uint32_t letter_uni)
{
    if(letter_uni < 256)
